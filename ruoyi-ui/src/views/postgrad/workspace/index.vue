      ...
      <div class="workspace-grid">
        ...
        <section class="workspace-panel matrix-panel">
          <div class="panel-heading">
            <div>
              <h3>学院 / 专业方向矩阵</h3>
              <span>{{ selectedSchool ? selectedSchool.name : '请选择学校' }}</span>
            </div>
              ...
              <el-table-column label="年份状态" min-width="210">
                <template slot-scope="scope">
                  <div class="year-cells">
                    <span v-for="year in yearOptions" :key="year" class="year-cell year-cell--pending">{{ year }}</span>
                  </div>
                </template>
              </el-table-column>
  ...
  computed: {
    kpiItems() {
      ...
      return [
        ...
        { key: 'college', label: '学院数', value: this.formatStat(stats.collegeCount), hint: '待聚合接口返回', tone: 'slate' },
    ...
    matrixEmptyTip() {
      ...
      return 'Phase 2 将提供 /postgrad/school/{id}/workspace 聚合数据。'
    }
  },
  created() {
    this.loadWorkspace()
  ...
  methods: {
    ...
    loadStats() {
      ...
      listWorkspaceStats(this.requestParams()).then(response => {
        ...
        this.stats = null
        this.statsNotice = '工作台聚合统计接口待接入，当前页面使用学校列表兜底展示。'
      }).finally(() => {
    ...
    loadSelectedSchool() {
      ...
      getSchoolWorkspace(this.selectedSchool.id, { year: this.filters.year }).then(response => {
        ...
        if (this.programRows.length) {
          this.selectedProgram = this.programRows[0]
        }
      }).catch(() => {
        this.workspace = null
      }).finally(() => {
    ...
    jumpToCrud(module) {
      this.$router.push({ path: '/postgrad/' + module })
    },
    jumpReview() {
      this.$router.push({
