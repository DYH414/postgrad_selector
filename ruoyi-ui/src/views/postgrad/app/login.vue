...
import { register } from '@/api/postgrad/appAuth'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      registerForm: { phone: '', email: '', password: '', confirmPassword: '' },
      loginRules: {
  ...
  methods: {
    ...mapActions('appUser', ['Login']),
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (!valid) return
        this.loading = true
        this.Login(this.loginForm).then(() => {
          this.$router.push(this.$route.query.redirect || '/app/profile')
        }).catch(() => {
    ...
    handleRegister() {
      this.$refs.registerForm.validate(valid => {
        ...
        register(this.registerForm).then(res => {
          ...
          this.loginForm.account = this.registerForm.phone || this.registerForm.email
        }).finally(() => { this.loading = false })
      })
    }
  }
